<?php
	...
	if($app->request()->isGet()){
		...
		if($row['type_of_participants'] == 0){
			...
			if($app->request()->get(Tags::$op) == Tags::$user){
				$query = "SELECT * FROM event_participants ep, users u, groups g WHERE ep.event=".$event." AND ep.status = 1 AND ep.participant = u.user_id AND ep.group_id =".$group." AND ep.group_id = g.group_id ";			
				//echo $query;				
				$result = $db->query($query);		
				echo json_encode($result->fetchAll(PDO::FETCH_ASSOC));				
			}
